...
- Wants hyper-realistic / photorealistic character rendering with transparent backgrounds (not cartoon or stylized); fish head should look like a beautiful prize-winning koi (luminous scales, vivid colors, glossy eye) -- natural fish beauty only, NO humanized features (no makeup, no eyelashes, no blush); clothes are 2D flat cartoon style with clean outlines; uses Gemini (`gemini-3-pro-image-preview`) via `GEMINI_API_KEY` with green-screen workaround (`#00FF00` bg + chroma-key removal) since Gemini cannot output true alpha
...
- Anchor character is a koi fish (錦鯉) in SIDE PROFILE (one eye visible, facing left, mouth pointing straight up) with @PufferNews-style proportions: massive head (~55-60% of character height), short stocky bust-only body (broad shoulders + upper chest, no waist/hips); on-screen handle is `@NewsCarp`; outfit is cobalt blue fitted feminine blazer (2D flat cartoon), black round-neck top, thin gold chain necklace; old pufferfish assets backed up to `mouth_backup_fish/`, previous v2 to `mouth_backup_v2/`
- Video template uses a layered composite: 2 mouth PNGs (mouth_0=closed, mouth_1=open) + desk overlay + randomly selected background; Remotion renders each frame independently in parallel during SSR -- all animation/lip-sync logic must be purely stateless (no `useRef` for cross-frame accumulation; it silently resets every frame); Remotion caches bundled assets in `/var/folders/.../T/remotion-*` -- must clear this cache between asset regeneration cycles or stale assets will appear in renders
